  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-200 p-8 font-sans selection:bg-purple-500/30">
      <div className="max-w-5xl mx-auto">
        ...
        <Link href="/" className="inline-flex items-center text-slate-400 hover:text-purple-400 text-sm font-bold uppercase tracking-widest transition-colors mb-8">
        ...
        <header className="bg-slate-900/40 backdrop-blur-md p-8 rounded-3xl border border-slate-800 mb-8 flex flex-col md:flex-row md:justify-between md:items-start gap-6">
          <div>
            <h1 className="text-4xl font-black text-slate-100 mb-3">{wedding.client_names}</h1>
            <div className="flex flex-wrap items-center text-slate-400 gap-6 text-sm font-medium">
              <span className="flex items-center"><Calendar className="w-4 h-4 mr-2 text-purple-400" /> {new Date(wedding.wedding_date).toLocaleDateString('cs-CZ')}</span>
              <span className="flex items-center"><MapPin className="w-4 h-4 mr-2 text-blue-400" /> {wedding.location_name || 'Lokalita neuvedena'}</span>
            </div>
          </div>
          <div className="md:text-right">
            <div className="text-xs text-slate-500 font-bold uppercase tracking-widest mb-2">Dohodnutá cena</div>
            <div className="text-3xl font-black text-emerald-400">{Number(wedding.price_czk).toLocaleString()} Kč</div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <section className="bg-slate-900/40 p-8 rounded-3xl border border-slate-800 focus-within:border-purple-500/50 transition-colors">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-bold text-slate-100 flex items-center">
                  <FileText className="w-5 h-5 mr-3 text-purple-400" /> Poznámky ze schůzky
                </h2>
                {savingNotes && <span className="text-xs text-slate-500 flex items-center"><Save className="w-3 h-3 mr-1 animate-pulse" /> Ukládám...</span>}
              </div>
              <textarea 
                value={notes}
            ...
            <section className="bg-slate-900/40 p-8 rounded-3xl border border-slate-800 focus-within:border-blue-500/50 transition-colors">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-bold text-slate-100 flex items-center">
                  <Clock className="w-5 h-5 mr-3 text-blue-400" /> Harmonogram dne
                </h2>
                {savingSchedule && <span className="text-xs text-slate-500 flex items-center"><Save className="w-3 h-3 mr-1 animate-pulse" /> Ukládám...</span>}
              </div>
              <textarea 
                value={schedule}
                ...
                className="w-full min-h-[250px] bg-transparent text-slate-300 placeholder-slate-600 outline-none resize-y leading-relaxed font-mono text-sm"
              />
            </section>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-800">
              ...
              <div className="space-y-4">
                {/* E-mail klienta */}
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">E-mail klienta</label>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl pl-9 pr-4 py-2.5 text-sm text-slate-300 placeholder-slate-600 outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50"
                    />
                  </div>
                </div>
                ...
                {/* Odkaz na galerii */}
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 block flex justify-between">
                    Odkaz na galerii
                    {savingGalleryUrl && <span className="text-xs text-slate-500 animate-pulse">Ukládám...</span>}
                  </label>
                  <div className="relative">
                ...
                {galleryUrl && clientEmail ? (
                  ...
                  <div className="text-xs text-slate-500 text-center italic mt-2 p-3 bg-slate-900/50 rounded-lg border border-slate-800/50">
            ...
            {/* Rychlé akce - Návrat tlačítka pro úpravu */}
            <Link 
              href={`/upravit/${wedding.id}`} 
              className="flex items-center justify-center gap-2 w-full px-4 py-4 bg-slate-800 text-slate-300 hover:text-white rounded-xl font-bold hover:bg-slate-700 transition border border-slate-700 mt-6"
            >
