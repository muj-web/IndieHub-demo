...
import { Plus, CheckCircle, Clock, CalendarDays, CheckSquare, Receipt, Sparkles, TrendingUp,MonitorSmartphone, Camera, Share2, ExternalLink, Target, Megaphone } from 'lucide-react';
import StrategyOnboarding from './StrategyOnboarding'; // Import našeho nového průvodce

export default function HomeModule() {
  const [stats, setStats] = useState({ activeWeddings: 0, newInquiries: 0, upcomingBookings: 0 });
  ...
  // Stavy pro Strategii a Tip
  const [userId, setUserId] = useState<string | null>(null);
  const [userStrategy, setUserStrategy] = useState<any>(null);
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (user) {
        setUserId(user.id);
        const { data: profile } = await supabase.from('profiles').select('marketing_strategy').eq('id', user.id).single();
        strategy = profile?.marketing_strategy;
        setUserStrategy(strategy);
      }

      // --- CHYTRÁ LOGIKA DENNÍCH TIPŮ ---
      const ALL_TIPS = [
        // 1. Tip, pokud vůbec NEMÁ strategii
        {
          id: 'no_strategy',
          title: 'Získej víc klientů',
          text: 'Mít super web je základ. Ale jak na něj dostat lidi? Pojďme si nastavit tvou obsahovou strategii.',
          icon: Target,
          color: 'text-emerald-600 dark:text-emerald-400',
          bg: 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/20',
          actionText: 'Spustit průvodce',
          actionAction: () => setShowStrategyWizard(true),
          condition: (s: any) => !s
        },
        // 2. Sociální sítě (Pokud je používá)
        {
          id: 'social_1',
          title: 'Aktivní sítě',
          text: 'Máš strategii zaměřenou na sociální sítě. Nezapomeň dnes přidat aspoň jedno Instastory. Co takhle ukázat "behind the scenes" ze tvé dnešní práce?',
          icon: Share2,
          color: 'text-pink-600 dark:text-pink-400',
          bg: 'bg-pink-50 dark:bg-pink-500/10 border-pink-200 dark:border-pink-500/20',
          condition: (s: any) => s?.has_socials === 'yes_active' || s?.focus === 'social'
        },
        // 3. Blog a SEO (Pokud chce psát)
        {
          id: 'seo_1',
          title: 'SEO: Čas na optimalizaci',
          text: 'Tento týden je ideální na to sepsat novou případovku! Otevři Google Search Console a podívej se, přes jaká slova tě lidé aktuálně hledají.',
          icon: TrendingUp,
          color: 'text-blue-600 dark:text-blue-400',
          bg: 'bg-blue-50 dark:bg-blue-500/10 border-blue-200 dark:border-blue-500/20',
          actionText: 'Otevřít Search Console',
          actionUrl: 'https://search.google.com/search-console',
          condition: (s: any) => s?.has_blog === 'yes_regular' || s?.focus === 'seo'
        },
        // 4. Doporučení (Pro ty, co nechtějí sítě ani psát)
        {
          id: 'word_of_mouth',
          title: 'Sbírej recenze',
          text: 'Protože se spoléháš hlavně na osobní doporučení, je kritické mít pětihvězdičkové recenze. Poslal jsi dnes klientům prosbu o hodnocení na Google?',
          icon: Megaphone,
          color: 'text-amber-600 dark:text-amber-400',
          bg: 'bg-amber-50 dark:bg-amber-500/10 border-amber-200 dark:border-amber-500/20',
          condition: (s: any) => s?.focus === 'word_of_mouth' || (!s?.has_socials && !s?.has_blog)
        },
        // 5. Univerzální tip
        {
          id: 'universal_1',
          title: 'Pravidelná revize webu',
          text: 'Zkus si dnes na mobilu otevřít svůj web a projdi si ho očima klienta. Fungují všechny odkazy? Jsou texty srozumitelné?',
          icon: MonitorSmartphone,
          color: 'text-purple-600 dark:text-purple-400',
          bg: 'bg-purple-50 dark:bg-purple-500/10 border-purple-200 dark:border-purple-500/20',
          condition: () => true // Spustí se pro kohokoliv
        }
      ];

      // Vyfiltrujeme jen ty tipy, které pro daného uživatele dávají smysl
      const applicableTips = ALL_TIPS.filter(tip => tip.condition(strategy));
      
      // Rotace tipů podle dne v roce (abychom ho nespamovali něčím jiným při každém F5)
      const dayOfYear = Math.floor((Date.now() - new Date(new Date().getFullYear(), 0, 0).getTime()) / 86400000);
      setDailyTip(applicableTips[dayOfYear % applicableTips.length]);

      // --- KLASICKÉ NAČÍTÁNÍ DAT ---
      ...
      const { data: upcomingWeds } = await supabase.from('weddings').select('*').gte('wedding_date', todayDateOnly).order('wedding_date', { ascending: true }).limit(3);
      const { data: bookings } = await supabase.from('bookings').select('*').neq('status', 'zruseno').gte('booking_date', todayIso).order('booking_date', { ascending: true }).limit(3);
      const { data: invoices } = await supabase.from('invoices').select('*, billing_clients(name)').eq('status', 'nezaplaceno').order('due_date', { ascending: true }).limit(3);
      const { data: activeTasks } = await supabase.from('tasks').select('*, projects(project_name)').eq('is_completed', false).order('created_at', { ascending: true }).limit(4);

      setStats({ activeWeddings: active, newInquiries: newInq, upcomingBookings: bookings?.length || 0 });
  ...
  return (
    <div className="animate-in fade-in duration-300">
      ...
      <header className="mb-8 flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-extrabold mb-1 text-zinc-900 dark:text-white tracking-tight">Vítej zpět.</h1>
      ...
      {/* --- BUBLINA: DENNÍ PARŤÁK --- */}
      {dailyTip && (
        <div className={`mb-8 p-6 rounded-3xl border ${dailyTip.bg} flex flex-col sm:flex-row items-start sm:items-center gap-6 relative overflow-hidden group transition-colors`}>
          <div className={`absolute -right-10 -top-10 w-40 h-40 blur-[50px] opacity-40 dark:opacity-20 ${dailyTip.bg.split(' ')[0]} pointer-events-none`} />
          
          <div className={`p-4 rounded-2xl bg-white dark:bg-slate-900/50 border border-zinc-200 dark:border-slate-800 shrink-0 ${dailyTip.color} shadow-sm`}>
            <dailyTip.icon className="w-6 h-6" />
          </div>
          
          <div className="flex-grow z-10">
            <div className="flex items-center gap-2 mb-1">
              <Sparkles className={`w-4 h-4 ${dailyTip.color}`} />
              <h3 className={`text-[10px] font-black uppercase tracking-widest ${dailyTip.color}`}>
                {userStrategy ? 'Tip pro dnešní den' : 'Chameleon Strategy'}
              </h3>
            </div>
            <p className="text-zinc-700 dark:text-slate-300 text-sm leading-relaxed mt-2 max-w-3xl font-medium">
              <strong className="text-zinc-900 dark:text-white font-black">{dailyTip.title}:</strong> {dailyTip.text}
            </p>
          </div>

          {(dailyTip.actionUrl || dailyTip.actionAction) && (
            <button 
              onClick={() => dailyTip.actionAction ? dailyTip.actionAction() : window.open(dailyTip.actionUrl, '_blank')}
              className={`shrink-0 px-6 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all hover:scale-105 flex items-center gap-2 z-10 bg-white dark:bg-slate-900 border border-zinc-200 dark:border-slate-700 hover:border-zinc-300 dark:hover:border-slate-500 ${dailyTip.color} shadow-sm`}
            >
              {dailyTip.actionText} <ExternalLink className="w-3.5 h-3.5" />
            </button>
          )}
        </div>
      )}

      {/* TOP KARTY SE STATISTIKAMI */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900/40 rounded-3xl border border-zinc-200 dark:border-slate-800 p-6 min-h-[200px] shadow-sm dark:shadow-none transition-colors">
