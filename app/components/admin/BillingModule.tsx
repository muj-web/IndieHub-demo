...
export default function BillingModule() {
  ...
  const handleInvoiceDelete = async (id: string) => {
    const isConfirmed = await confirm({
      ...
      message: 'Opravdu chcete tuto fakturu trvale smazat?',
  ...
  return (
    <div className="animate-in fade-in duration-300">
      ...
      {/* POS TERMINÁL (Modal) - Zůstává nezměněn */}
      {isPosOpen && (
      ...
      {/* HLAVIČKA - PŘESNĚ PODLE FUNKČNÍHO DEMA (flex + flex-1 pro 2 a 2 tlačítka na mobilu) */}
      <header className="mb-10 flex flex-col xl:flex-row xl:justify-between xl:items-end gap-6 w-full">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold mb-1 text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-orange-500 dark:from-amber-400 dark:to-orange-400">Fakturační přehled</h1>
          <p className="text-zinc-500 dark:text-slate-400 text-sm font-medium">Správa dokladů, záloh a klientů</p>
        </div>
        
        {/* Tlačítka - zabalená do bloků s flex-1 aby na mobilu dělaly pěkné dva řádky */}
        <div className="flex flex-col md:flex-row gap-3 w-full xl:w-auto">
          {/* První řádek (nebo levá polovina na desktopu) */}
          <div className="flex gap-3 w-full md:w-auto">
            <Link href="/prehled" className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-zinc-100 dark:bg-slate-900 hover:bg-zinc-200 dark:hover:bg-slate-800 text-zinc-700 dark:text-slate-300 rounded-xl font-bold transition-colors text-sm border border-zinc-200 dark:border-slate-800 shadow-sm">
              <TrendingUp className="w-4 h-4 shrink-0" /> Přehled
            </Link>
            <Link href="/klienti" className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-zinc-100 dark:bg-slate-900 hover:bg-zinc-200 dark:hover:bg-slate-800 text-zinc-700 dark:text-slate-300 rounded-xl font-bold transition-colors text-sm border border-zinc-200 dark:border-slate-800 shadow-sm">
              <UserSquare className="w-4 h-4 shrink-0" /> Adresář
            </Link>
          </div>
          {/* Druhý řádek (nebo pravá polovina na desktopu) */}
          <div className="flex gap-3 w-full md:w-auto">
            <button onClick={() => setIsPosOpen(true)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-zinc-900 dark:bg-white hover:bg-zinc-800 dark:hover:bg-zinc-200 text-white dark:text-zinc-900 rounded-xl font-bold transition-all text-sm shadow-lg">
              <Smartphone className="w-4 h-4 shrink-0" /> Platba
            </button>
            <Link href="/nova-faktura" className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-amber-600 hover:bg-amber-500 text-white rounded-xl font-bold transition-colors text-sm shadow-lg shadow-amber-500/20 whitespace-nowrap">
              <Plus className="w-4 h-4 shrink-0" /> Faktura
            </Link>
          </div>
        </div>
      </header>

      {/* STATISTIKY - Responzivita převzatá z funkčního dema */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-8 w-full">
        {[
          { label: 'Obrat (30 dní)', value: `${invoices.filter(i => i.status === 'zaplaceno').reduce((s, i) => s + Number(i.total_amount), 0).toLocaleString('cs-CZ')} Kč`, icon: <DollarSign className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-500 dark:text-emerald-400" /> },              
          { label: 'Nezaplaceno', value: invoices.filter(i=>i.status==='nezaplaceno').length, icon: <Clock className="w-5 h-5 sm:w-6 sm:h-6 text-amber-500 dark:text-amber-400" /> },
          { label: 'Po splatnosti', value: invoices.filter(i=>i.status==='po_splatnosti').length, icon: <AlertTriangle className="w-5 h-5 sm:w-6 sm:h-6 text-rose-500 dark:text-rose-400" /> },
          { label: 'Vystaveno celkem', value: invoices.length, icon: <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-blue-500 dark:text-blue-400" /> },
        ].map((stat, i) => (
          <div key={i} className="bg-white dark:bg-slate-900/40 p-4 sm:p-6 rounded-2xl border border-zinc-200 dark:border-slate-800/50 flex items-center justify-between shadow-sm dark:shadow-none">
            <div className="min-w-0 pr-2">
              <div className="text-xs sm:text-sm text-zinc-500 dark:text-slate-400 mb-1 truncate">{stat.label}</div>
              <div className="text-xl sm:text-3xl font-black text-zinc-900 dark:text-slate-100 truncate">{stat.value}</div>
            </div>
            <div className="p-2 sm:p-3 bg-zinc-50 dark:bg-slate-800/50 rounded-xl border border-zinc-100 dark:border-slate-700/50 shrink-0">{stat.icon}</div>
          </div>
        ))}
      </div>

      {/* FILTRY - PŘESNĚ PODLE FUNKČNÍHO DEMA (flex-wrap + flex-grow) */}
      <div className="flex flex-wrap md:flex-nowrap gap-2 md:gap-3 mb-8 bg-white dark:bg-slate-900/30 p-2 rounded-2xl border border-zinc-200 dark:border-slate-800/50 w-full items-center shadow-sm dark:shadow-none">
      ...
      {/* TABULKA - PŘESNĚ PODLE FUNKČNÍHO DEMA */}
                ...
                <tr key={inv.id} className="hover:bg-zinc-50 dark:hover:bg-slate-800/30 transition-colors group">
                  <td className="px-6 py-4 font-bold text-zinc-900 dark:text-slate-200">
                    <div className="text-sm flex items-center gap-2">
                      {inv.invoice_number}
                      {inv.type === 'proforma' && <span className="text-[9px] bg-zinc-100 dark:bg-slate-800 text-zinc-500 dark:text-slate-400 px-1.5 py-0.5 rounded border border-zinc-200 dark:border-slate-700 font-black">ZÁLOHA</span>}
                    </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-3">
                     <button onClick={() => handleInvoiceDelete(inv.id)} className="p-2 text-zinc-400 dark:text-slate-500 hover:text-rose-500 dark:hover:text-rose-400 hover:bg-zinc-100 dark:hover:bg-slate-800 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                     <Link href={`/upravit-fakturu/${inv.id}`} className="p-2 text-zinc-400 dark:text-slate-500 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-zinc-100 dark:hover:bg-slate-800 rounded-lg transition-colors"><Edit3 className="w-4 h-4" /></Link>
                     <Link href={`/faktura/${inv.id}`} className="p-2 bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-500 hover:bg-amber-100 dark:hover:bg-amber-500/20 hover:text-amber-700 dark:hover:text-amber-400 rounded-lg transition-colors"><Eye className="w-4 h-4" /></Link>
                   </div>
                 </td>
                </tr>
              ))
