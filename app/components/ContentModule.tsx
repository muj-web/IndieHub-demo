...
import { Plus, Trash2, Edit3, Image as ImageIcon, Calendar as CalendarIcon, LayoutGrid, ChevronLeft, ChevronRight, X, Save, Share2 } from 'lucide-react';
...
export default function ContentModule() {
  ...
  // Stav pro vyskakovací okno (Modal)
  ...
  // --- UPRAVENO: Místo promptu otevřeme náš hezký modal jako "nový" draft ---
  const addQuickIdea = () => {
    setEditingPost({
      title: '',
      content: '',
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setEditingPost({ 
        ...editingPost, 
        media_link: [...currentLinks, ...newUrls] 
      });
  ...
  // --- UPRAVENO: Logika uložení Modalu (pozná Nový vs Úpravu) ---
  const handleSavePost = async () => {
    ...
    // 1. Záchrana: Název nesmí být úplně prázdný
    if (!payload.title || !payload.title.trim()) {
      alert("Vyplň alespoň název příspěvku.");
      return;
    }

    // 2. Pokud payload NEMÁ ID, znamená to, že zakládáme úplně nový příspěvek
    if (!payload.id) {
    ...
    // 3. Pokud payload MÁ ID, tak existující příspěvek pouze aktualizujeme
    else {
  ...
  // --- ODESLÁNÍ DO MAKE.COM (Z předchozí opravy) ---
  const handlePublishToMake = async () => {
    if (!editingPost) return;

    const WEBHOOK_URL = process.env.NEXT_PUBLIC_MAKE_WEBHOOK_URL || ''; 
    if (!WEBHOOK_URL) {
      alert("Chyba: Není nastavena adresa pro Make.com webhook v .env.local");
      return;
    }
        ...
        <div key={d} className={`min-h-[120px] border border-slate-800 p-2 transition-colors hover:bg-slate-900/30 ${isToday ? 'bg-emerald-500/5 border-emerald-500/30' : ''}`}>
          ...
          <div className="space-y-1">
            {postsThisDay.map(post => (
              <button key={post.id} onClick={() => setEditingPost(post)} className="w-full text-left text-[9px] p-1.5 rounded bg-purple-500/20 border border-purple-500/30 text-purple-300 truncate font-bold uppercase hover:bg-purple-500/40 transition-colors">
                {post.title}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="animate-in fade-in duration-300 relative">
      ...
      <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <button onClick={addQuickIdea} className="bg-purple-600 hover:bg-purple-500 text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center transition-colors shadow-lg shadow-purple-500/20">
          <Plus className="w-4 h-4 mr-2" /> Nový příspěvek
        </button>
      </header>
            ...
            <div key={col.id} className="bg-slate-900/40 rounded-3xl border border-slate-800 p-5 min-h-[500px] flex flex-col">
              ...
              <div className="space-y-4 flex-grow">
                {posts.filter(p => p.status === col.id).map(post => (
                  <div key={post.id} className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800/50 group hover:border-purple-500/50 transition-all shadow-sm">
        ...
        <div className="fixed inset-0 z-[200] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center">
              {/* Dynamická hlavička podle toho, zda upravujeme, nebo tvoříme */}
              <h3 className="text-xl font-black text-white">
                {editingPost.id ? 'Upravit příspěvek' : 'Nový nápad'}
              </h3>
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-grow custom-scrollbar">
              ...
              {/* Text */}
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Text příspěvku (Copy)</label>
                <textarea value={editingPost.content || ''} onChange={(e) => setEditingPost({...editingPost, content: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-4 text-slate-200 focus:outline-none focus:border-purple-500 transition-all min-h-[150px] resize-y placeholder:text-slate-700" placeholder="Sem napiš svůj text..." />
              </div>
            </div>
            ...
            <div className="p-6 border-t border-slate-800 bg-slate-900/50 flex justify-between items-center rounded-b-3xl">
              <div>
                {/* Tlačítko pro Make.com se zobrazí, jen pokud příspěvek už existuje v databázi (má ID) */}
                {editingPost.id && (
                  <button onClick={handlePublishToMake} className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl font-bold text-sm transition-colors shadow-lg shadow-blue-500/20 flex items-center">
                    <Share2 className="w-4 h-4 mr-2" /> Vydat přes Make.com
                  </button>
                )}
              </div>
              <div className="flex gap-4">
                ...
                <button onClick={handleSavePost} className="bg-purple-600 hover:bg-purple-500 text-white px-6 py-2.5 rounded-xl font-bold text-sm flex items-center transition-colors shadow-lg shadow-purple-500/20"><Save className="w-4 h-4 mr-2" /> Uložit</button>
              </div>
            </div>
          </div>
        </div>
