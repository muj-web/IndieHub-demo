...
export default function SuperadminModule() {
  ...
  async function fetchSuperadminData() {
    ...
    // 1. Načteme profily a projekty
    const { data: profiles, error: pError } = await supabase
      .from('profiles')
      .select('*, projects(id, status)');

    if (pError) {
      console.error("❌ Superadmin: Chyba při načítání profilů:", pError);
    } else {
      setClients(profiles || []);
      
      // Výpočet statistik
      if (profiles) {
          const totalProjs = profiles.reduce((acc, p) => acc + (p.projects?.length || 0), 0);
          const demoProjs = profiles.reduce((acc, p) => 
            acc + (p.projects?.filter((prj: any) => prj.status === 'rozpracovano').length || 0), 0);
