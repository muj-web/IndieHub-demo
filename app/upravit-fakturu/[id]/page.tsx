...
export default function UpravitFakturu() {
  ...
  const [invoice, setInvoice] = useState({
    ...
    bank_account: '',
    iban: '',
    status: 'nezaplaceno' // Přidali jsme i status, abys ho mohl rovnou měnit
  });

  const [items, setItems] = useState<any[]>([]);

  // 1. Načtení dat faktury a seznamu klientů
  useEffect(() => {
    const fetchInvoiceData = async () => {
      // Načtení klientů do roletky
      const { data: clientsData } = await supabase.from('billing_clients').select('*').order('name');
      if (clientsData) setClients(clientsData);

      // Načtení samotné faktury
      ...
      if (itemsData && itemsData.length > 0) {
        ...
        // Fallback pokud by faktura neměla položky (nemělo by se stát)
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    // 1. Aktualizace hlavičky faktury
    ...
    // 2. Aktualizace položek je složitější. Nejbezpečnější je staré položky smazat a nahrát upravené znovu.
    ...
    // Přesun zpět na dashboard nebo detail
    router.push('/');
    router.refresh();
  };

  if (loading) return <div className="p-10 text-center text-slate-400 bg-slate-950 min-h-screen">Načítání faktury...</div>;
    ...
    <main className="min-h-screen bg-slate-950 p-8 font-sans text-slate-200 selection:bg-amber-500/30">
      <div className="max-w-4xl mx-auto bg-slate-900/50 p-8 rounded-3xl shadow-sm border border-slate-800">
        ...
        <header className="flex items-center gap-4 mb-8 border-b border-slate-800/50 pb-6">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-100">Upravit fakturu</h1>
            <p className="text-sm text-slate-400 font-medium mt-1">Číslo dokladu: {invoice.invoice_number}</p>
          </div>
        </header>
        ...
        <form onSubmit={handleUpdate} className="space-y-8">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="text-[10px] font-black uppercase text-slate-500 ml-1 block mb-2">Klient</label>
              <select 
            ...
            <div>
              <label className="text-[10px] font-black uppercase text-slate-500 ml-1 block mb-2">Stav faktury</label>
              <select 
              ...
              {items.map((item, index) => (
                <div key={item.id} className="flex gap-3 items-start">
                  <div className="flex-grow">
                    ...
                    <input type="text" required placeholder="Popis..." value={item.description} onChange={(e) => updateItem(item.id, 'description', e.target.value)} className="w-full p-3 bg-slate-900 border border-slate-800 rounded-xl text-sm text-slate-200 focus:ring-1 focus:ring-amber-500 outline-none" />
                  </div>
                  <div className="w-24">
                  ...
                  <div className="w-32">
                    {index === 0 && <label className="text-[10px] font-black uppercase text-slate-500 ml-1 block mb-1">Cena za ks</label>}
          ...
          <div className="bg-slate-900/80 p-6 rounded-2xl border border-slate-800 flex justify-between items-center mt-8">
            <span className="text-slate-400 uppercase font-black tracking-widest text-sm">Nová celková cena</span>
